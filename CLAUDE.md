...
- Offline episodes: playback always uses presigned R2 URLs, but offline SAVES go through the authenticated same-origin GET /audio/{slug} streaming proxy (storage.open_stream) because JS can't fetch() cross-origin presigned URLs. Saved audio lives in Cache Storage "stackdock-audio-v1" under the STABLE key /offline-audio/{slug} (presigned URLs expire; the key doesn't) with metadata in localStorage offline:{slug}. The /offline page is deliberately UNAUTHENTICATED + self-contained (inline CSS/JS, zero server data — content comes from the device's own caches) and is precached; offline navigations fall back to it. CRITICAL: the SW activate handler must never delete AUDIO_CACHE — keep the k !== AUDIO_CACHE guard or users lose their flight downloads on every deploy. navigator.storage.persist() is requested on first save to reduce eviction on Android.
...
- Custom /404 page (Tumblr-style, self-contained, tiled bg + figure + embedded YouTube audio attempt); signup page meme copy. SW STATIC_CACHE is at v11 — bump it when static assets change.
...
- GitHub Actions use Node 20 (actions/checkout@v4, actions/setup-python@v5), deprecated; bump when convenient.
