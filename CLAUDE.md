...
app/radio.py             Member radio (/radio, nav '📻 Radio'): anyone submits a song (text
                         ...
                         appends the same way, /radio/playnext cuts to the front, ↑/↓ and
                         DRAG (pointer events, not HTML5 DnD — that never fires on touch;
                         the ⠿ handle sets touch-action:none so a drag doesn't scroll)
                         reorder via POST /radio/reorder,
...
app/plex.py              Plex tab (/plex): libraries, recently-added, search,
                         ...
                         the PLEX server, not the droplet; per-user+item session param or
                         concurrent transcodes kill each other; Safari/iOS play HLS natively,
                         Chrome/Firefox via VENDORED app/static/hls.min.js (v1.5.20 — Plex
                         serves CORS for it, verified; same architecture as Plex's own web app).
                         Stream URLs carry the PER-SERVER token (accepted tradeoff for a
                         trusted member group; NOT the admin account token — a shared server
...
- Mobile nav (Aug 2026 UX experiment; git tag `restore-pre-ux` = the pre-experiment state): on <=640px the top `.tabs` row is hidden and a fixed bottom dock (`.dock` in base.html — Articles/Podcasts/Yours/Saved, active state set by a tiny inline script from ?tab=/pathname) replaces it; `main` gets bottom padding clearing the dock + safe-area. The Your Stuff sub-tabs render as a full-width segmented control (`.pub-filter.subtabs` overrides) at every viewport. The masthead nav also collapses on <=640px: wordmark + ☰ burger (body.nav-open toggles), and links the dock already covers (Articles/Podcasts/Saved, class `dock-dup`) are hidden on phones entirely. The theme `<select>` MARKUP lives inside the masthead (position:fixed ignores DOM placement, so desktop keeps its bottom-right float) — on phones it goes static and appears inside the open ☰ menu (a floating picker blocked the burger; logged-out mobile pages therefore have no picker, acceptable).
- db.init() merges case-variant publication/show names (per-post name stamping splits a source into two chips when a Substack pub re-capitalizes itself — GrimDark/GrimdarkEnlightenment, Aug 2026): each case-insensitive group merges to its newest item's spelling, follows migrate with OR IGNORE.
...
- Custom /404 page (Tumblr-style, self-contained, tiled bg + figure + embedded YouTube audio attempt); signup page meme copy. SW STATIC_CACHE is at **v35** — bump it when static assets change.
...
- Position-sync 401 hardening (July 2026): sessions now have SLIDING expiry (session_refresh middleware in main.py re-signs the cookie once >24h old — auth.SESSION_REFRESH_AFTER; skips responses that already Set-Cookie so logout isn't resurrected). The July 2026 data-loss chain it fixes: fixed 30-day cookies expired on installed PWAs → every /api/positions call 401ed SILENTLY → both flushers treated the failed GET as an EMPTY server and the "local done but server not-done → defer to server" branch DELETED local done flags. Flushers (base.html + offline.html) now ABORT on a non-ok GET (base.html also shows a fixed "session expired — tap to log in" banner on 401). done-flags are promoted pending→"1" only when the push returns ok (pushServer/pushPos resolve a delivered? bool; sendBeacon resolves false since it can't confirm) — never from navigator.onLine, which says "online" even when the session is dead.
- iOS Media Session (July 2026): iOS DROPS the Now Playing / lock-screen entry after pausing web audio (~30s on iOS 17/18; user-reported IMMEDIATE on iOS 26 — WebKit deactivates its shared AVAudioSession; Apple forums thread 762582, WebKit bug 261858 — not preventable via Media Session APIs). Aug 2026, USER-APPROVED reversal of the old "no silent-audio keepalive" stance: episode.html now runs a BOUNDED pause keepalive — on pause (engaged, not ended) it loops a runtime-generated silent WAV (silent SAMPLES, not volume=0, which iOS ignores) for a 5-min grace window so lock-screen resume keeps working, then releases audio focus. The keeper runs from FIRST PLAY onward (not started at pause time — a lock-screen pause happens on a hidden page where starting NEW audio can be refused; already-playing audio just continues), stops 5 min after pause / on done / ended / pagehide. CRITICAL keeper consequence (device-trace-proven, Aug 2026): iOS keeps its OWN playing/paused model of the session, updated by the action it last sent AND by what it actually hears — the keeper makes those disagree, so the model drifts arbitrarily: traces showed "play" delivered while playing, "pause" while paused, inverted lock-screen icons, and iOS once pausing the element itself with no tap. Resolution: BOTH ms action handlers ("play" and "pause") are one TOGGLE (every tap flips playback regardless of the icon shown), and ms.playbackState declares what iOS HEARS, not the true state — "playing" whenever keeperAlive (silent loop audible through a pause), "paused" only once genuinely silent (stopKeeper re-declares) — because declaring "paused" while the keeper played is what made the model oscillate. Cosmetic cost accepted: during the 5-min grace the lock screen shows the ⏸/playing presentation while actually paused (scrubber frozen via setPositionState playbackRate 0 while paused; rate-0 try/caught with rate-1 fallback). Tradeoffs accepted: holds audio focus + battery during the window. Not yet added to the /offline shelf player. DEAD END — do NOT retry (tried + reverted 2 Aug 2026, commits 506c1b5/0358183): replacing the audible <audio> keeper with a silent running AudioContext (gain-0 ConstantSource) to get truthful icons DETACHES Media Session action delivery entirely on iOS — with the ctx running, lock-screen taps stopped reaching the page even during normal playback (device trace: ctx running, locked, then zero ms actions). The audible-keeper + both-actions-toggle state is the verified functional ceiling for a web app; the icon sometimes lying during the grace window is the unavoidable cosmetic cost (every tap still does the right thing). If icons ever matter more than lock-screen resume, the alternatives are removing the keepalive or native podcast apps via the private RSS feed. Remote diagnostics are PERMANENT and app-wide (user request, Aug 2026 — installed iOS web apps can't be remotely inspected): `window.diag` in base.html HEAD keeps a localStorage-persisted event ring (`diag:pending`, cap 200, survives crashes/offline sessions, entries carry pathname + "[hidden]" tags) flushed to POST /api/diag/{key} (auth-required; LOGGED to docker logs only, never stored) on visibility change/pagehide (hooks registered in the BOTTOM script so page scripts log their own visibility events first) + a boot "leftovers" flush. Auto-captured: JS errors, unhandled rejections, positions-GET failures, done-flag drops/push failures, player events (play/pause/ms actions/keeper outcomes/presign reloads/offline-save failures). The /offline page appends to the SAME ring via its self-contained `olog` but never flushes itself (must work with zero network); base.html ships its entries next signed-in page. Read with `docker compose logs stackdock | grep 'PLAYER DIAG'`. Add `diag.log(...)` calls freely when debugging device-side behavior. The players mitigate: navigator.audioSession.type="playback" (Safari 16.4+), setPositionState kept fresh (play/pause/seeked/ratechange + ~4s timeupdate throttle — required for a live lock-screen scrubber, esp. at non-1× speed), playbackState mirrored, artwork declared at BOTH 128x128 and 512x512 (same src; pre-iOS-18 compact player needs the small entry), and the whole session (metadata+handlers+position) RE-ASSERTED on visibilitychange→visible / pageshow so controls rebuild the moment the app is reopened (that reopen requirement is the platform behavior, not a bug in our player).
- Patreon (app/ingest/patreon.py, hourly): per-member `session_id` cookie at /accounts, fetched via curl_cffi Chrome impersonation (bare session — extra headers like Referer break the Cloudflare fingerprint). Stream posts + per-campaign BACK-CATALOG sweeps (fetch_campaign_posts via /api/posts?filter[campaign_id], up to PATREON_CAMPAIGN_BACKFILL_POSTS=500) because the stream only surfaces recent posts — "the MAN report" (June 2026, patreon.com/bst = Brad Troemel) was invisible until this existed. Sweep state in `patreon_campaigns` table, re-swept every PATREON_BACKFILL_REFRESH_DAYS=7 so a new pledge/free-trial unlocks old locked stubs (backfill inserts are always notified=1/silent). CRITICAL flag lesson: Patreon `is_paid` does NOT mean patron-gated (gated posts routinely report is_paid=false; it's pay-per-post) — access truth is `current_user_can_view`; we store locked = !can_view, is_paid = is_paid OR locked. Free trials can be EXCLUDED from some posts by the creator (can_view=false despite an active trial) — that's Patreon, not a bug. FREE-TIER PROMO FILTER (Aug 2026): can_view=true posts whose served body is a membership upsell ("You're currently on the free tier..." — Brad Troemel blasts these constantly) are detected by _is_tier_promo (phrase list _PROMO_PHRASES, first 500 chars of rendered text) and stored hidden=1 + locked=1 + silent; a paying account's later sync upgrades the body and auto-unhides (only rows whose STORED body matches the promo pattern — admin hides never resurface). 11 pre-existing Brad Troemel rows were backfilled hidden directly in the prod DB. Video posts (video_external_file) become articles with media_key=post id, played via /media/{slug} → fresh signed Mux HLS per play. scripts/debug_patreon.py still exists for cookie probing (env PATREON_SESSION, never CLI/chat).
- /help member guide (July 2026, auth-required, nav link "Help"): PWA install steps for iOS/Android + the substack.sid cookie walkthrough + offline tips. Illustrated with inline theme-aware SVG mockups in help.html (CSS vars, no new static assets, FAKE placeholder cookie values — never commit real DevTools screenshots; they'd contain a live session cookie). Header wordmark + logo sit STRAIGHT by default now (resting tilts removed from .wordmark/.brand-chud; the hover wiggle remains, chud-mode still suppresses it).
