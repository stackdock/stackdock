                    ...
                    <div className="bg-blue-50 p-4 rounded">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div className="bg-white p-3 rounded border text-center">
                                <div className="text-2xl font-bold text-blue-600">{schedulesData.data.length}</div>
                                <div className="text-sm text-gray-600">Sites with Backups</div>
                            </div>
                            <div className="bg-white p-3 rounded border text-center">
                            ...
                            <div className="bg-white p-3 rounded border text-center">
                                ...
                                <div className="text-sm text-gray-600">Remote Backups</div>
                            </div>
                            <div className="bg-white p-3 rounded border text-center">
                                <div className="text-2xl font-bold text-gray-600">
                                    {schedulesData.data.reduce((sum, site) => sum + site.schedule_backups.filter(s => s.type === 'local').length, 0)}
                                </div>
                                <div className="text-sm text-gray-600">Local Backups</div>
                            </div>
                        </div>
                    </div>
                        ...
                        {schedulesData.data.length > 0 ? (
                            <div className="space-y-4">
                                {schedulesData.data.map(siteSchedule => (
                                    <div key={siteSchedule.site_id} className="bg-white p-4 rounded border">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            {siteSchedule.schedule_backups.map(schedule => {
                                                ...
                                                return (
                                                    <div key={schedule.id} className="border rounded p-3 bg-gray-50">
                                                        <div className="flex items-start justify-between mb-2">
                                                            <div className="flex items-center gap-2">
                                                                <span className="text-lg">{typeInfo.icon}</span>
                                                                <div>
                                                                    <h4 className="font-semibold">{typeInfo.name}</h4>
                                                                    <p className="text-xs text-gray-600">ID: {schedule.id}</p>
                                                                </div>
                                                            </div>
                                                            <span className={`px-2 py-1 rounded text-xs font-medium ${frequencyInfo.color}`}>
                            ...
                            <div className="text-center py-8">
                                ...
                                <p className="text-sm text-gray-400 mt-1">Add backup schedules to protect your sites</p>
                            </div>
                        )}
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                {Object.entries(
                                    ...
                                    return (
                                        <div key={frequency} className="flex items-center justify-between p-2 bg-white rounded border">
                                            <span className="flex items-center gap-2">
                                                <span>{info.icon}</span>
                                                <span>{info.name}</span>
                                            </span>
                                            <span className={`px-2 py-1 rounded text-sm ${info.color}`}>
                                                {count}
                                            </span>
                                        </div>
                                    );
