            ...
            {backupData && (
                <div className="space-y-4">
                    ...
                    <div className="bg-blue-50 p-4 rounded">
                        <h2 className="font-semibold text-lg mb-2">API Response Status</h2>
                        <div className="flex items-center gap-4">
                            <div className={`px-3 py-1 rounded font-semibold ${
                                backupData.success ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                            }`}>
                                {backupData.success ? '✅ Success' : '❌ Failed'}
                            </div>
                            <p className="text-gray-700">{backupData.message}</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-green-50 p-4 rounded">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="bg-white p-3 rounded border text-center">
                                <div className="text-2xl font-bold text-blue-600">{backupData.integrations.length}</div>
                        ...
                        {backupData.integrations.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {backupData.integrations.map(integration => {
                                    ...
                                    return (
                                        <div key={integration.id} className="bg-white p-4 rounded border">
                                            <div className="flex items-start justify-between mb-3">
                                                <div className="flex items-center gap-3">
                                                    <div>
                                                        <h3 className="font-bold">{integration.integration_name}</h3>
                                                        <p className="text-sm text-gray-600">ID: {integration.id}</p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="space-y-2">
                                                <div className="grid grid-cols-1 gap-2">
                                                    <div className="flex justify-between items-center">
                                                        ...
                                                        <span className="text-sm text-gray-600">{integration.integrated_service}</span>
                                                    </div>
                                                    <div className="flex justify-between items-center">
                                                        <span className="text-sm font-medium">Region:</span>
                                                        <span className="text-sm text-gray-600">{integration.region}</span>
                                                    </div>
                                                </div>

                                                <div className="pt-2 border-t">
                                                    <h4 className="text-sm font-semibold mb-1 text-gray-700">Credentials</h4>
