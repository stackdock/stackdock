...
# Periodic sweep that picks up any queued download the on-demand trigger missed
# (queued while a run held the lock, or stranded 'downloading' by a restart).
MDE_POLL_MINUTES = int(os.getenv("MDE_POLL_MINUTES", "5"))
# Separate, slower cadence: refresh the browse catalogue cache so the /mde tab is
# current without a cold fetch on open (list_series + episodes; see mde.refresh).
MDE_CATALOGUE_MINUTES = int(os.getenv("MDE_CATALOGUE_MINUTES", "60"))
