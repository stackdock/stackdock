...
export const autoSyncAllDocks = internalAction({
  handler: async (ctx) => {
    // CRITICAL: Wrap entire function in try-catch to ensure scheduler loop never stops
    // Even if there's an error, we must schedule the next run
    try {
      const now = Date.now()
      ...
      // CRITICAL: Even if there's a fatal error, we must schedule the next run
      // Otherwise the sync loop will stop permanently
      console.error(`[Auto-Sync] ❌ FATAL ERROR in sync cycle:`)
      console.error(`[Auto-Sync]    ${error instanceof Error ? error.message : String(error)}`)
      console.error(`[Auto-Sync]    Stack: ${error instanceof Error ? error.stack : "N/A"}`)
      console.error(`[Auto-Sync] ⚠️  Scheduling next sync despite error to keep loop alive...`)
      
      // Schedule next sync even after fatal error (use longer delay to avoid rapid retries)
      await ctx.scheduler.runAfter(60 * 1000, internal.docks.scheduled.autoSyncAllDocks)
    }
  },
})
...
export const initializeAutoSync = internalMutation({
  handler: async (ctx) => {
    ...
    // Check if any dock has recent sync activity (within last 5 minutes)
    // Use 5 minutes instead of 2 to account for longer sync intervals
    ...
    if (hasRecentSync) {
      console.log(`[Auto-Sync] Sync already initialized (recent sync activity detected within last 5 minutes)`)
      // Force restart anyway to ensure loop is running (scheduler might have stopped)
      console.log(`[Auto-Sync] Force restarting sync loop to ensure it's running...`)
      await ctx.scheduler.runAfter(0, internal.docks.scheduled.autoSyncAllDocks)
      return
    }

    // Schedule first sync immediately
    console.log(`[Auto-Sync] No recent sync activity - scheduling first sync immediately...`)
    await ctx.scheduler.runAfter(0, internal.docks.scheduled.autoSyncAllDocks)
