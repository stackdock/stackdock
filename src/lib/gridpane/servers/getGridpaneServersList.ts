...
export async function getGridPaneServersList(page: number = 1): Promise<ServersResponse> {
  ...
  const requestUrl = `${url}/server?page=${validPage}`;
  ...
  try {
    ...
    const response = await withRetry(
      async () => {
        return await fetchWithTimeout(requestUrl, {
          ...
          next: {
            revalidate: 60,
            tags: [`gridpane-servers`, `gridpane-servers-page-${validPage}`]
          }
        });
      },
      endpoint,
    ...
    const data = await handleGridPaneResponse<ServersResponse>(response, endpoint, validPage);
    ...
    logApiCall(`${endpoint} (${data.data.length} servers)`, validPage, duration);
