---
title: Scaffold CLI with OpenTUI Terminal UI Framework
labels: enhancement,cli,high-priority
priority: high
...
Replace the basic Commander.js CLI implementation with a rich terminal user interface (TUI) using [OpenTUI](https://github.com/sst/opentui), providing an interactive, modern command-line experience for StackDock users.
...
**Reference**: [OpenTUI GitHub Repository](https://github.com/sst/opentui)

## Why OpenTUI?

OpenTUI is a TypeScript library for building terminal user interfaces (TUIs) that provides:

1. **Modern TUI Framework**: Built specifically for terminal interfaces with React/SolidJS reconcilers
2. **TypeScript Support**: Full type safety and modern tooling
3. **Standalone Core**: `@opentui/core` works completely standalone with imperative API
4. **React Integration**: `@opentui/react` reconciler available for React-based UI components
5. **Active Development**: Maintained by SST team, actively developed
6. **Production Ready**: Used by SST's own tools (opencode, terminaldotshop)
...
3. **Install Zig (Required for OpenTUI)**
...
4. **Create TUI Application Structure**
...
- [StackDock CLI README](../packages/cli/README.md)
